...
vec4 effect(vec4 vcolor, Image tex, vec2 tex_coords, vec2 pixel_coords)
{
	...
	vec3 c01 = Texel(tex, tex_coords - dx).xyz;
	vec3 c02 = Texel(tex, tex_coords + dg2).xyz;
	vec3 c10 = Texel(tex, tex_coords - dy).xyz;
	vec3 c11 = Texel(tex, tex_coords).xyz;
	vec3 c12 = Texel(tex, tex_coords + dy).xyz;
	vec3 c20 = Texel(tex, tex_coords - dg2).xyz;
	vec3 c21 = Texel(tex, tex_coords + dx).xyz;
