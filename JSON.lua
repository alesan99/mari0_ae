...
function OBJDEF:onDecodeError(message, text, location, etc)
   ...
   if text then
	  if location then
		...
		local s = ""
		while true do
			i, endfind = string.find(text, "\n", i+1)    -- find 'next' newline
			if i == nil then break end
			if i > location then
				s = text:sub(finds[math.max(1, #finds-2)], endfind)
				break
			end
			table.insert(finds, i)
			linenum = linenum + 1
		end
		ogmessage = string.format("%s \n at line %d \n at char %d \n \nlocation:%s", message, linenum, location, s)

         message = string.format("%s at char %d of: %s", message, location, text)
      else
		ogmessage = string.format("%s: %s", message, text)
   ...
   --AE Addition
   --idk how to modify cause it uses complicated meta tables so im just going to change this lua
  	if JSONcrashgame then
		if self.assert then
		...
		JSONerror = {message, ogmessage, text, location, etc}
		assert(false, message) --need to use this so pcall works
	end
end
   ...
   elseif type(value) ~= 'table' then
      ...
      if not object_keys then
         ...
         for _, key in ipairs(object_keys) do
			...
			--AE ADDITION (REMOVES EXTRA SPACES)
